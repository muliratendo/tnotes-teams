...
// The QuickNoteFunc type is an adapter to allow the use of ordinary
// function as QuickNote mutator.
type QuickNoteFunc func(context.Context, *ent.QuickNoteMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f QuickNoteFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.QuickNoteMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.QuickNoteMutation", m)
}
