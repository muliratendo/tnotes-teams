...
func (cc *CommentCreate) SetCardID(u uuid.UUID) *CommentCreate {
...
func (cc *CommentCreate) SetCard(c *Card) *CommentCreate {
	return cc.SetCardID(c.ID)
}
...
func (cc *CommentCreate) check() error {
	...
	if _, ok := cc.mutation.CardID(); !ok {
		return &ValidationError{Name: "card_id", err: errors.New(`ent: missing required field "Comment.card_id"`)}
	}
	if _, ok := cc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "Comment.created_at"`)}
	}
	if len(cc.mutation.CardIDs()) == 0 {
		return &ValidationError{Name: "card", err: errors.New(`ent: missing required edge "Comment.card"`)}
	}
	return nil
}
...
func (cc *CommentCreate) createSpec() (*Comment, *sqlgraph.CreateSpec) {
	...
	if nodes := cc.mutation.CardIDs(); len(nodes) > 0 {
